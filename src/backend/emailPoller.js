...
async function getOrCreateBurnerAccount() {
  try {
    ...
    console.warn('[EmailPoller Warning] Could not auto-provision Ethereal test account:', err.message);
    return { user: 'manual.paste@deltafareguard.local', pass: '', host: 'local', port: 993 };
  }
}
...
async function pollBurnerInbox() {
  ...
  try {
    ...
    if (!account || account.host === 'local') return;
    ...
    try {
      for await (let message of client.fetch({ seen: false }, { source: true })) {
        ...
        try {
          await importFlightFromReceipt(emailText, message.uid.toString());
        } catch (importErr) {
...
/**
 * Initializes continuous background polling in non-blocking manner
 */
function initEmailPoller() {
  setTimeout(() => {
    ...
    }).catch(e => console.warn('Email poller async init warning:', e.message));
  }, 2000);
}
